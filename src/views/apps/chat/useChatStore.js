...
export const useChatStore = defineStore("chat", {
  ...
  actions: {
    async fetchChatsAndContacts(q) {
      ...
      const res = await axios.get(`/owner/chats_and_contacts?search=${q}`);
      console.log(res);
    ...
    async sendMsg(message) {
      ...
      const  data  = await axios.post(`/owner/create_chats`,{
        message:message,
        super_admin_id:this.activeChat?.data?.super_admin.id  
      })
      console.log(data)
