...
const contact = computed(() => ({
  id: store.activeChat.data.super_admin.id,
  avatar: store.activeChat?.data.super_admin.avatar,
}));
...
const msgGroups = computed(() => {
  ...
  if (store.activeChat) {
    ...
    let msgSenderId = store?.activeChat?.data?.chat[0]?.owner_id;
    let msgGroup = {
      ...
      <div>
        <div class="" >
          ...
          <p v-if="msgGrp.messages.length > 0" :class="[
              msgGrp.messages[msgGrp.messages.length - 1].type !== 1
